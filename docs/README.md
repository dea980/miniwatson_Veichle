...
> MiniWatson(watsonx 클론: data·ai·governance)을 **자동차 도메인 특화 LLM 플랫폼**으로 확장한 프로젝트.
...
| [INFERENCE_OPTIMIZATION.md](INFERENCE_OPTIMIZATION.md) | 양자화·추론최적화 + 벤치 + 자원 직렬화 |
| [AGENT.md](AGENT.md) | Agentic Search(라우팅·SQL 자기수정·모델라우팅) + 진단서·이미지진단·부품견적 |
...
| [../ml/data/sources.md](../ml/data/sources.md) | 데이터 소스·라이선스 |

## 3. 현재 진행 현황 (V-트랙)

- [x] **V1 데이터 수집** — NHTSA 리콜 119건(원본 JSON 보존) + Internet Archive 매뉴얼 PDF 3종
- [x] **V2 자동차 RAG** — `vehicle` 네임스페이스, accent 123청크, 한국어 답변·근거(sources) 검증
- [x] **V3 리콜 text-to-SQL** — `/api/tabular`(DuckDB)
- [x] **V4 LoRA 파인튜닝** — MLX, early-stopping(iter-200), base vs FT
- [x] **V5 추론 최적화** — 양자화 벤치(Q4 vs Q8: 메모리½·속도2배) → Ollama 서빙
...
1. **데이터 소스 전환** — 현대차 공홈(oms.hmc.co.kr)은 로그인·ToS 제약 → **공개·재현가능** 소스로:
...
4. **데이터 품질이 9할** — 약한 생성모델(granite4)로 만든 합성데이터가 음역오류·환각("하다모"·"이등신") → 학습 시 그대로 주입. → 생성 선생모델을 qwen2.5:7b로 키우고 발췌근거·고유명사 규칙 강제. (= "데이터 기반 파인튜닝" 역량)
...
7. **자원 직렬화** — 단일 M2에서 생성·학습·추론 동시 = 메모리 초과/크래시. → 무거운 작업 하나씩, `ollama stop`으로 회수. *운영에선 별도 노드 분리* (분리 아키텍처의 실증 근거).
...
| 경량화·추론최적화(온디바이스) | ml/optimize (GGUF Q4 + 벤치) | 🔄 |
