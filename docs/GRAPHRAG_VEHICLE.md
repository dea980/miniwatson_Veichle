...
> 자동차 도메인에서 벡터 RAG가 약한 **연결형, 멀티홉 질의**를 메우기 위한 경량 GraphRAG 설계.
...
- **연결**: "안전벨트 관련 이슈를 **매뉴얼, 리콜, 불만을 통틀어** 정리" — 세 소스가 *같은 부품/시스템*으로 엮여야 답이 됨.
...
- **엔티티 = `automotive-glossary.json` 사전 + 규칙(DTC 정규식 `P\d{4}`, 부품과 시스템 용어)** — 추출 비용 ~0, 결정적.
...
- **효과**: 연결형, 멀티홉 질문 recall↑, 답변에 **그래프 경로**를 근거로 제시 → 설명가능성↑.
...
> "벡터와 BM25로는 부족한 **관계 추론**을 GraphRAG로 메운다. 단 로컬 제약상 LLM triple 추출 대신 **도메인 사전(자동차 약어와 DTC) 기반 공출현 그래프**로 비용 0에 결정적으로 구현하고, RRF 3번째 소스로 합류시켜 매뉴얼, 리콜, 불만을 부품 단위로 잇는다. on/off A/B로 효과를 정량화한다."
