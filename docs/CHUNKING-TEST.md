...
- 테스트 문서: sample/rag-notes.txt (주제가 개념 / 4단계 파이프라인 / 장점과 비용으로 나뉘어 경계 비교에 적합)
...
1. yaml: strategy: fixed -> 저장
2. 재시작: ./mvnw spring-boot:run
3. 업로드 + 청크 확인:
...
1. yaml: strategy: recursive -> 저장
2. 재시작
3. 업로드 (namespace=test-recursive):
...
1. yaml: strategy: semantic -> 저장
2. 재시작
3. 업로드 (namespace=test-semantic):
