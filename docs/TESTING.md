...
| IngestionServiceTest | IngestionService.formatOf | 확장자->추출기 라우팅(.hwp/.hwpx->전용, 그 외->Tika), 대소문자와 null과 복합확장자 — HWP가 Tika로 새는 회귀 방지 |
...
prefixFor는 원래 private였으나, 테스트 가능성을 위해 package-private static 순수 함수로 뺐다(`record Prefix(q, d)` 반환). 같은 패키지의 EmbeddingServiceTest가 Spring 컨텍스트와 Ollama 없이 매핑만 직접 검증한다 — 이 prefix가 틀어지면 에러 없이 검색 점수만 조용히 깎이는, 공정 비교의 핵심 로직이라 단위 테스트로 못 박았다.
