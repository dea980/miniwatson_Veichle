...
디자인 카드(내가 만듦)가 진짜 스크린샷을 액자처럼 감싸는 구조. 표지에서 시선 잡고 → 진짜 도는 화면으로 신뢰 주고 → 숫자로 닫는다.

| # | 카드 | 출처 | 캡션(슬라이드에 안 넣어도 됨, 흐름용) |
|---|---|---|---|
| 1 | `card1_architecture.png` | 생성됨 (`docs/assets/linkedin/`) | 표지 + 3계층 매핑 — "뭘 만들었나" 한 장 |
| 2 | Ask + Sources 스크린샷 | 네 스크린샷 크롭 | "답변에 출처가 붙는다 (RAG grounding)" |
| 3 | Audit Trail 스크린샷 | 네 스크린샷 크롭 | "모든 LLM 호출이 감사로그에 남는다" — **차별화 포인트** |
| 4 | `card4_results.png` | 생성됨 | 숫자 4개 + GitHub CTA |

**2·3번 크롭 가이드 (Mac):** `Cmd+Shift+4`로 해당 패널만 캡처하거나, 전체 스크린샷을 Preview에서 열어 해당 영역만 잘라라.
- 카드 2: **Ask 패널** 전체 (질문 입력줄 → 답변 박스 → SOURCES 목록까지). 가장 직관적인 hero.
- 카드 3: **Audit Trail 탭의 테이블** (ID/Question/Model/Latency/PII/Sources/Timestamp 행). 여기가 핵심.
- 카드 1·4가 1080×1080(정사각)이니, 스크롭한 2·3도 **1:1로 맞춰라** — 잘라낸 패널을 흰 배경 1080 정사각 캔버스 가운데 올리면 4장이 한 세트로 보인다. (Preview에서 캔버스 키우거나, 원하면 내가 맞춰줄게.)

**AVG LATENCY 4932ms** 다시 한 번 질문 던져 낮은 수치로 갱신하거나 크롭에서 빼는 걸 추천.
