...
MiniWatson Vehicle을 데모에서 운영으로 올릴 때의 위협 모델, 적용한 통제, 잔여 리스크를 기록한다. 핵심 원칙은 **"테넌트 격리는 구조가 아니라 강제(enforcement)로 완성된다"**이다 — namespace로 데이터를 나눠도, 그걸 강제하는 인증/인가가 없으면 보안상 의미가 없다.

§1~7은 기반 플랫폼에서 상속한 **테넌트 격리와 인증** 보안 코어(도메인 무관, 그대로 유효)다. **§8은 자동차 도메인 특화 보안과 프라이버시**(차량 PII, Agent/SQL 인젝션, 데이터 거버넌스, H-Chat 정합)를 다룬다 — vehicle 트랙에서 새로 부각되는 표면이다.

## 1. 위협 모델

보호 대상(asset):

- 테넌트별 청크 본문과 임베딩(article_vectors, parquet) — 경쟁사나 부서 간 유출 시 치명적.
...
| T3 | 시크릿 하드코딩(pg 비번 `miniwatson`) | **완화**: prod는 비번/API키 env와 시크릿 주입, 폴백 제거(미주입 시 기동 실패) | 높음 |
...
|---|---|---|---|
| 인증 | `X-API-Key` 헤더 → config 맵 | 같은 키, SS 필터체인 | `Bearer <JWT>` 서명과 클레임 |
...
- **B** — Spring Security 필터체인과 인증 컨텍스트를 제대로 학습. API key는 SS 1급 시민이 아니라 커스텀 필터를 끼워야 해 약간 어색.
- **C** — 실제 엔터프라이즈(watsonx IAM, OAuth2)에 가장 근접, 확장성과 표준성 최고. 대신 토큰 발급기, JWKS, 클레임 매핑까지라 로컬 학습엔 무겁다.

채택: **격리 코어 공통 → A로 working 확인 → B → C** 순으로 같은 토대에 어댑터를 더한다. 코어가 셋의 90%라, A를 세우면 B와 C는 인증 앞단만 교체.
...
| 검증 범위 | 키 문자열 대조 | 키 문자열 대조 | **서명과 만료까지**(HS256/JWKS) |
...
### JWT(C)란 — 토큰 민팅
JWT = `header.payload.signature`(점 구분, base64). payload에 신원과 권한 claim(`sub`, `namespaces`, `exp`), signature는 `HMAC-SHA256(header.payload, secret)`로 위조 방지.

- **운영**: IdP(Keycloak/Auth0/IBM IAM)가 로그인 후 토큰을 **민팅(발급)**하고 앱(C)은 **검증만**(서명과 만료). 앱이 비밀번호를 안 봐서 키 회전과 SSO에 강함.
...
검증 완료: A와 C 라이브 **401/200/403** 확인(C는 `eval/test_jwt_auth.sh` — HS256 서명 검증 + namespaces claim 격리). B는 A와 동일 키 대조 패턴.

### 운영 전환
- A: 키 회전 직접 구현, 내부 API 한정 권장.
- B: SS 표준 체인, 키는 여전히 직접 관리.
- C: prod는 대칭키(`jwt-secret`) 대신 `spring.security.oauth2.resourceserver.jwt.issuer-uri`(IdP JWKS)로 교체 → 키 회전과 만료를 IdP가 담당.
...
- **키 저장.** 데모는 config 맵이지만, 운영은 **해시 저장**(평문 금지) + 회전과 폐기 절차. 로테이션 없는 정적 키는 유출 시 영구 노출.
...
2. **`TenantGuard` (순수 인가 규칙) 분리** — "이 namespace 봐도 되나"의 규칙만. Spring, DB, HTTP 의존 0이라 **단위테스트로 규칙을 고정**(이미 5케이스: 허용/거부/와일드카드/미인증/blank). 규칙이 한 곳에 있어 변경과 감사가 쉬움. prefixFor, requireReadOnly와 동일한 "순수 static" 패턴.
...
5. **강제 지점 = 데이터 경계** — 컨트롤러가 아니라 namespace를 받는 서비스(RagService.ask, IngestionService.*)에서 가드를 부른다. 새 컨트롤러나 새 진입점이 생겨도 **데이터 계층에서 한 번 막히면 끝** — 강제점이 흩어지지 않는다(단일 진실). 질의뿐 아니라 적재도 같은 지점에서 막아 "남의 namespace에 쓰기"까지 차단.
...
- **누가 쓰는지 = 스코핑과 감사.** 로그인이 세션을 사람/테넌트에 묶어, 보여줄 namespace를 결정하고 "누가 무엇을"을 감사에 남긴다.
...
- Tier 2 (다음): 입력검증과 프롬프트 인젝션 문서화(T4/T5), PII 커버리지(T6), TLS와 레이트리밋(T7).
- Tier 3: 감사 완전성(주체 연결), 의존성 스캔. 인증 어댑터 B(Spring Security+API key)와 C(JWT) 추가.

## 6. 잔여 리스크 (정직하게)

- A 방식은 키 회전과 폐기를 직접 구현해야 하며 표준 프레임워크의 검증된 방어가 빠진다 → 운영은 B/C로.
- 프롬프트 인젝션은 완전 차단이 어렵다(검색 콘텐츠는 신뢰 불가 입력) — 시스템 프롬프트 경계와 출력 검증으로 완화.
...
## 8. 자동차 도메인 보안과 프라이버시 (Vehicle)

§1~7의 격리와 인증 코어 위에, 자동차 트랙에서 **새로 부각되는 표면**을 정리한다. 플랫폼에선 가설(T4/T5)이거나 일반 PII(T6)였던 것이 vehicle에선 **주경로**가 된다.

### 8.1 차량 개인정보(PII) — 마스킹 커버리지 확장

자동차 데이터는 일반 PII(이메일, 전화) 외에 **차량 특유 식별자**를 담는다. 특히 성능·상태점검기록부(자동차관리법 별지 제82호)나 견적, 진단 흐름에서 그렇다.

| 식별자 | 예 | 리스크 | 대응 |
|---|---|---|---|
| VIN(차대번호) | `KMHxxxxxxxxxxxxxx` (17자리) | 차량과 소유주 추적 | 정규식 마스킹 추가 필요 |
| 번호판 | `12가3456` | 개인 식별 | 한국 번호판 패턴 마스킹 |
| 차주 성명, 연락처 | 점검기록부 기재 | 직접 식별정보 | 기존 PII 규칙 + 이름 필드 |
| 정비 이력, 위치 | 입고 정비소, 주행거리 | 행동 추적 | 로그 적재 전 검토 |

> 현재 `PiiRedactionService`는 이메일, 전화, SSN, 카드 중심(플랫폼 상속)이다. **VIN, 번호판, 차주명은 미커버** → vehicle 배포 전 정규식 룰 추가가 Tier 2 항목이다. 데모 데이터(NHTSA, 샘플 CSV)는 실제 개인정보를 담지 않아 데모 자체는 안전하나, 운영(실제 점검기록부 인제스트)에선 필수다.

### 8.2 Agent / text-to-SQL 인젝션 — 이제 주경로 (T4, T5 격상)

플랫폼에선 부차적이던 두 위협이 vehicle의 **핵심 기능 경로**라 우선순위가 올라간다.

- **text-to-SQL 인젝션(T5)** — `/api/tabular`, `/api/agent`가 자연어를 SQL로 자동 생성한다. 가드는 **SELECT 전용**(DDL/DML 차단), DuckDB는 **읽기 전용 인메모리**(CSV만), 식별자 정규화(`normalize_names`)로 임의 테이블 접근을 차단한다. 자기수정 재시도도 같은 SELECT 가드 안에서만 돈다. 데이터 변조 표면은 막혀 있으나, **민감 컬럼 노출**(있다면)은 컬럼 화이트리스트로 추가 방어를 권장한다.
- **프롬프트 인젝션(T4)** — Agent가 검색된 매뉴얼 청크나 이미지 OCR 텍스트를 프롬프트에 넣는다. 신뢰할 수 없는 입력이 "도구를 호출하라" 또는 "다른 namespace를 봐라" 식으로 오염시킬 수 있다. 완화책은 시스템 프롬프트 경계를 고정하고, 도구 선택을 **규칙 기반 라우터**(키워드 우선, LLM은 폴백)로 둬서 모델이 임의 도구를 못 부르게 하고, 출력은 결정적 SQL/집계로 검증하는 것이다. 완전 차단은 불가능하므로(잔여 리스크) 라우터의 결정성이 1차 방어가 된다.

### 8.3 데이터 거버넌스 — 출처와 라이선스 분리

자동차 코퍼스는 **공개 가능성이 제각각**이라 적재와 커밋 정책을 분리한다.

- **NHTSA**(리콜, 불만) — 미국 공공데이터라 키가 불필요하고 재배포가 가능해 샘플 CSV를 커밋한다.
- **매뉴얼 PDF**(Internet Archive 등) — 저작권이 있어 `data/vehicle/`(gitignore)에 두고 **로컬 인덱싱만** 하며 원문은 커밋하지 않는다.
- **성능점검기록부** — 표준 양식만 참조하고, **실제 기재 데이터는 개인정보**라 합성/샘플만 사용한다.
- 현대차 공홈(oms.hmc.co.kr) 대량 크롤링은 **ToS와 저작권** 위반 소지가 있어 회피한다(수동 다운로드 + 스크립트 인제스트). 근거는 [ml/data/sources.md](../ml/data/sources.md).

거버넌스 관점에서는 "무엇을, 어디서, 재배포 가능한지"를 출처별로 명시한다. 감사 로그에 source-type을 남겨 답변 근거의 라이선스를 추적할 수 있다.

### 8.4 멀티프로바이더 / H-Chat 게이트웨이 정합

`LlmClient` 추상화로 ollama|watsonx|vertex|bedrock|azure|vLLM를 `llm.provider`로 교체한다. 현대 **H-Chat**(사내 LLM 게이트웨이) 같은 환경에선:

- 모든 LLM 호출이 **단일 게이트웨이를 통과**하므로 감사, PII 마스킹, 정책을 한 지점에서 강제한다(현재 거버넌스 레이어가 그 choke point).
- 온디바이스(차량)와 클라우드 경로를 전환해도 **거버넌스 통과 지점은 불변**이다. 추상화 덕에 provider만 바뀌고 감사/마스킹 코드는 그대로다.
- 데이터 주권: 민감 질의는 온프레미스/온디바이스 모델로, 일반 질의는 클라우드로 라우팅하는 정책을 provider 스위치로 구현할 수 있다.

### 8.5 vehicle 보안 로드맵 (Tier 2 추가분)

- VIN, 번호판, 차주명 PII 정규식 추가(`PiiRedactionService` 확장)와 점검기록부 인제스트 전 마스킹 검증.
- text-to-SQL 민감 컬럼 화이트리스트(노출 컬럼 제한).
- Agent 프롬프트 경계 강화와 도구 호출 화이트리스트 문서화.
- 감사 로그에 source-type과 라이선스 태그 + 호출 주체 연결(§4.1).
