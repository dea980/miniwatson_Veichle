...
골든셋은 "쉬운 질문 모음"이 아니라, 검색과 생성의 서로 다른 능력과 알려진 약점을 의도적으로 겨냥한 묶음이다. 각 category가 노리는 것:
...
같은 정답셋이 영어 3개 + 한국어 5개 namespace에 걸쳐 있어, 포맷과 언어별 검색 강도를 한 번에 본다. (표 데이터 csv/xlsx는 RAG가 아니라 SQL 트랙으로 분리.)
...
표 데이터의 정밀 질의와 집계 질의는 RAG가 아니라 SQL로 채점한다(`run_eval.py --sql`). 각 케이스는 `{table, path, question, expect}` (xlsx는 제목행 skip용 `headerRow` 포함)이고, `/api/tabular/load` 후 `/ask`로 LLM이 만든 SQL을 실행해 `expect` 값이 결과 행에 있는지 본다.
...
집계(AVG/SUM/COUNT)는 벡터 RAG가 원천적으로 못 하는 영역이라 SQL 트랙으로 분리했다(이유와 구조는 [TABULAR-SQL.md](TABULAR-SQL.md)). xlsx(kr-housing)도 `registerXlsx`(POI→임시 CSV) 추가로 이제 SQL 트랙에서 채점한다.
...
이 오버라이드는 **평가 전용**이다. 프로덕션에서 외부 요청이 검색 전략을 마음대로 바꾸면 일관성과 보안 문제가 된다. 그래서:
