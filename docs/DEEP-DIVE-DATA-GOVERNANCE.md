...
- **쓰기**는 hot(JSON)에만 append → 임계치(기본 100, `storage.tier.threshold`)에서 cold로 compact.
...
### 4.1 🔴 `augmentedPrompt`가 마스킹 없이 저장된다 (PII 누수)
...
### 4.2 🔴 감사 쓰기 실패가 사용자 요청을 깨뜨린다 (실패 격리 없음)
...
### 4.3 🟠 로깅이 RAG/비-RAG를 구분 못 한다 (의미 혼선)
...
### 4.7 🟡 우→소 결합: 프레젠테이션 annotation이 영속성에 누수 (Data)
