...
## 5.4 GPU 예산 / "현재 영역으로 되나"

비전 경로(③)에 필요한 자원과, GPU 없이 가능한 범위.

**③ 비전 RAG에 필요한 GPU — 데이터센터급 아님:**

| 단계 | 모델 | 4bit VRAM | 돌아가는 곳 |
|---|---|---|---|
| 인덱싱(페이지 임베딩) | ColQwen2 ~2–3B | ~2–4GB | Colab T4(무료)·RTX 3060급 |
| 답변(VLM이 페이지 읽음) | Qwen2-VL 7B | ~6–8GB | 16GB GPU 1장 |

소규모는 **16GB GPU 1장(또는 Colab T4)** 으로 충분. 70B급 고품질 VLM이라야 A100 — 그건 과함.

**현재 영역(M2, GPU 없음, Ollama+llava)으로:**

| 요소 | 현재 영역 | 결론 |
|---|---|---|
| **표** | ✅ 됨 — 구조추출은 비전·GPU 불필요 | ①로 처리 |
| **다이어그램** | △ llava로 *돌긴* 함 | 정확도가 llava 한계(②에서 막힌 그 지점) |

다이어그램 선택지(요약): (1) **로컬 llava on-demand** — GPU 0, 정확도 낮음 / (2) **클라우드 VLM 1콜**(Qwen-VL/Gemini, 다이어그램 질문에만) — GPU 소유 0·정확도↑·데이터 외부 전송(주권 트레이드오프) / (3) **ColQwen 자체호스팅** — 16GB GPU 1장.
