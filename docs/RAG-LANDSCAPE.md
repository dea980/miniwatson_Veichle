...
| Agentic RAG | 에이전트가 도구와 다단계 검색 오케스트레이션 | 多 | ✗ |
...
**인프라:** pgvector/Postgres가 이미 있으니 벡터 검색과 메타 필터는 OK. 그래프가 필요하면 **Apache AGE 확장**으로 같은 Postgres에서 처리 가능(Neo4j 신규 도입 불필요). RAPTOR 트리 노드는 `level` 필드를 가진 벡터로 기존 저장소에 그대로 적재 가능.
