...
> MiniWatson(watsonx 클론, data·ai·governance)을 **자동차 도메인 특화 LLM 플랫폼**으로
...
- 공홈 대량 크롤링은 ToS·저작권 리스크 + JS 렌더로 불안정.
- → **공개·재현가능** 소스로: NHTSA(공개 API), Internet Archive(공개 PDF), 공공데이터포털(리콜 OpenAPI, 사용자 키 발급 필요).
...
- LoRA = 전체 가중치 대신 저랭크 어댑터만 학습 → 작은 데이터·메모리로 도메인 말투/형식 주입.
...
| SOTA 조사·오픈소스 벤치마킹 | 모델 비교 + benchmark | 🟡 일부 |
...
**완료(Day 1):** 리콜 119건 수집(원본 JSON 보존) · 매뉴얼 PDF 다운로드 · accent 123청크 vehicle 인제스트 · 도메인 RAG+한국어 답변 검증 · text-to-SQL 트랙 확보.
...
| 영역 | 선택 | 이유 |
|---|---|---|
| 본체 | Spring Boot 4 / Java 21 | 기존 견고한 서빙·거버넌스 자산 |
...
| Base LLM(파인튜닝) | Qwen2.5-1.5B-Instruct | Apache-2.0, 로컬 학습·추론 현실적 |
