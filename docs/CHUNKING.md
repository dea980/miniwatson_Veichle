...
1. 임베더 토큰 한도. nomic-embed-text는 약 8k 토큰까지만 본다. 90k자 문서는 앞부분만 임베딩되고 뒷부분은 잘려서 벡터에 반영되지 않았다. 문서 뒷부분(임팩트 수치 등)을 묻는 질의가 매칭되지 않았다.

2. 단일 벡터로는 부분 질의를 못 잡는다. 문서 전체를 한 벡터로 뭉개니 특정 구절과의 유사도가 흐려지고, 짧고 선명한 다른 문서가 더 가깝게 잡히는 역전이 일어났다.
...
1. 문장 단위 분리
2. 각 문장을 임베딩
3. 인접 문장 간 cosine 유사도 계산
4. 유사도가 percentile 임계값 아래로 떨어지는 지점(주제 전환) = 경계
5. 경계 사이 문장을 묶되 최대 크기를 넘으면 강제 분할
...
ingestText는 List<Article>을 반환한다. 같은 baseTitle(번호 제거)로 묶어 문서 단위 작업(요약)을 처리하고, 같은 namespace에 같은 baseTitle이 이미 있으면 재삽입하지 않는다(dedup).
...
트레이드오프는 ingest 비용 vs 경계 품질 하나다. 2.4 측정 기준:

| 상황 | 선택 | 근거 |
|---|---|---|
| 기본값 | recursive | 비용 0, 경계 보존. 이유 없으면 이걸로 |
| 경계 품질이 결정적, ingest 느려도 됨 | semantic | 의미 단위 묶음. 문장마다 임베딩이라 850자에 3.45s |
| 구조 없는 텍스트, 속도 우선 | fixed | 가장 빠름. 단어 잘림 감수 |

max-size는 별개 파라미터다. 실험은 250, 실사용은 800~1000. 청크가 임베더 토큰 한도(nomic ~8k) 안에만 들어오면 된다.

기본은 recursive, semantic은 특정 문서에만 골라 쓴다. 교차 비교는 [DECISIONS.md](DECISIONS.md).

## 6. 다음 단계

- reranking. 청크가 101개로 늘었으므로 1차로 넉넉히 top-N 후보를 뽑고 reranker로 최종 top-K만 LLM에 전달
