...
## 벡터 검색 확장 사다리 + load-once 캐시

### 발견한 버그 (load-all-per-request)

진단서를 느린 대형 모델(gemma4)로 돌릴 때 로그에 `[Parquet] loadAll → 357 articles`가 **요청마다, 동시 요청이면 여러 스레드에서 동시** 찍혔다. 콜드 티어(`ArticleParquetStore.loadAll`)가 읽기 경로(문서 목록, 카운트, reindex, 벡터 인덱스 하이드레이트)마다 **Parquet 전체를 디스크에서 다시 읽고** 있었다. 모델 지연과 겹치자 응답이 누적 지연 → 요청 타임아웃(docs/DEBUGGING.md 4.11).

### 수정 (load-once 캐시)

`ArticleParquetStore`에 인메모리 캐시를 둔다: **디스크 읽기는 최초 1회**, 이후엔 캐시 반환, 쓰기(`saveAll`) 시 캐시 갱신, `synchronized`로 동시 요청 시 단일 읽기 보장, 호출부 변형 대비 **방어적 복사본** 반환. 357행은 수 MB라 메모리에 두는 게 정상 — 문제는 "크기"가 아니라 "매 요청 재읽기"였다.

### 확장 사다리 (규모별 올바른 선택)

| 단계 | 검색/저장 | 적합 규모 | 비고 |
|---|---|---|---|
| 인메모리(현재) | Parquet hydrate + 인메모리 벡터 인덱스 | 수백~수천 청크 | **load-once 캐시 필수** |
| pgvector (이미 구현) | Postgres + HNSW 서버측 ANN | 수만~수십만 | `vector.store=pgvector`로 스왑. **전체를 메모리에 안 올림** |
| lakehouse | 오브젝트스토리지(S3/MinIO) + 파티션 Parquet + DuckDB/Trino | 수백만+ | watsonx.data식 분산 |

### 판단 ("분산화가 필요한가?")

수백 행 규모에서 **분산은 증상의 해법이 아니다** — 분산해도 "매 요청 재읽기"는 그대로다(오히려 네트워크 홉↑). 올바른 순서는 ① load-once 캐시(완료) → ② 진짜 확장은 **pgvector**(이미 보유, 서버측 ANN이라 전체 로드 불필요) → ③ 대규모는 lakehouse(로드맵). 357행에 분산 인프라를 까는 것은 규모에 안 맞는 오버엔지니어링이다.

교훈: **"UI 500/느림 = 데이터가 큼"이 아니다.** 먼저 *접근 패턴*(매 요청 전체 재로드)을 의심하라. 캐시 한 줄이 분산 인프라보다 먼저다.
