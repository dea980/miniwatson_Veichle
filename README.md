...
> **자동차 도메인 특화 LLM 플랫폼** — 정비 매뉴얼 RAG · 리콜/불만 text-to-SQL · 온디바이스 LoRA 파인튜닝 · Agentic 진단→부품 견적 · 거버넌스까지 한 곳에서.
>
> Spring Boot 4 · Next.js · Ollama · DuckDB · MLX(LoRA) · Qwen2.5 / IBM Granite
>
> 현대차 NLP/LLM 직무 JD(도메인 특화 LLM 최적화 · 경량화/추론 · RAG/Agent 고도화 · 밸류체인 NLP · MLOps)에 매핑한 엔드투엔드 구현.
...
- **리콜·불만 text-to-SQL** — NHTSA 리콜/불만·부품 CSV를 자연어로 질의(DuckDB), 집계·차트. SQL 자기수정 내장.
- **온디바이스 LoRA 파인튜닝** — Qwen2.5-1.5B를 자동차 도메인으로(MLX, GPU 없는 맥) → GGUF Q4 양자화 → Ollama 서빙.
- **Agentic Search** — 질문→도구선택(RAG / 리콜SQL / 복합)→실행→한국어 종합 + 트레이스.
- **차종 종합 진단서** — 리콜·불만·매뉴얼을 한 리포트로 종합(집계는 결정적 SQL, 서술은 LLM).
- **이미지 진단→필요 부품** — 차량 사진(Vision + OCR)→매뉴얼 진단→부품 명세 + 샘플 견적.
- **거버넌스** — 모든 LLM 호출 감사 로그 · PII 마스킹 · 멀티프로바이더(현대 H-Chat 게이트웨이 정합).

> 데모 화면·정량 결과: **[docs/RESULTS.md](docs/RESULTS.md)** · 설계 근거: **[docs/VEHICLE_ARCHITECTURE.md](docs/VEHICLE_ARCHITECTURE.md)** · 전체 문서 색인: **[docs/README.md](docs/README.md)**
...
자동차 밸류체인 NLP을 **서빙(Java) ↔ 모델 제작(Python)** 으로 분리하고, 완성 모델을 `llm.provider`로 갈아끼운다 = 온디바이스↔클라우드 대응.
...
상세는 [docs/VEHICLE_ARCHITECTURE.md](docs/VEHICLE_ARCHITECTURE.md).

---

## ML 파이프라인 (`ml/`)

GPU 없는 맥(M2) 제약 → **작은 모델 + LoRA + 4bit 양자화 = 온디바이스** 컨셉. 이 제약이 곧 JD "차량 온디바이스 추론 최적화" 스토리.
...
quantize_gguf.sh            # GGUF Q4_K_M 양자화 → Ollama 등록 (네이티브 safetensors import)
        ↓
benchmark.py                # TTFT · tok/s · 메모리 (Q4 vs Q8: 메모리½ · 속도≈2배)
```
...
> 현대차 공홈(oms.hmc.co.kr)은 로그인·ToS·JS렌더 제약 → **공개·재현가능** 소스로 대체. 매뉴얼 원문·원본 JSON은 `data/vehicle/`(gitignore), 샘플 CSV만 커밋. 실행 단계는 [ml/RUNBOOK.md](ml/RUNBOOK.md), 소스·라이선스는 [ml/data/sources.md](ml/data/sources.md).
...
1. **인제스트** — 정비 매뉴얼 PDF 업로드 (namespace=`vehicle`)
2. **질문** — "P0420 코드가 뭐야?" → 매뉴얼 근거 한국어 답변 + 출처
3. **리콜 SQL** — "팰리세이드 리콜 몇 건이야?" → DuckDB 집계
4. **진단서** — 차종 입력 → 리콜·불만·매뉴얼 종합 리포트(PDF 출력)
5. **이미지 진단** — 차량 사진 업로드 → 진단 + 필요 부품 + 샘플 견적
...
# 부품 견적 (부품 선택=LLM, 금액 계산=Java 결정적)
...
매뉴얼 RAG·파일 업로드·거버넌스 등 플랫폼 공통 API는 [docs/API.md](docs/API.md) 참고.

---

## 로드맵 (Vehicle 트랙)

- [x] **V1 데이터 수집** — NHTSA 리콜/불만 API(원본 JSON 보존) + Internet Archive 매뉴얼 PDF (`ml/data/fetch_*.py`)
- [x] **V2 자동차 RAG** — `vehicle` 네임스페이스 + 약어/DTC 시드 사전 + 한국어 답변·근거
- [x] **V3 리콜 text-to-SQL** — `/api/tabular` (DuckDB)
- [x] **V4 도메인 LoRA** — Qwen2.5-1.5B (MLX, early-stopping) + base vs FT 평가
- [x] **V5 경량화** — GGUF Q4 → Ollama 서빙 + 벤치(Q4 vs Q8: 메모리½·속도≈2배)
- [x] **V6 Agentic Search** — 도구선택(RAG/SQL/복합) + 종합 + 트레이스 (`/api/agent`)
- [x] **V7 UI** — Next.js 7탭 + 브라우저 음성(STT/TTS)
- [x] **V8 멀티테이블** — NHTSA 불만 추가 + 설정 레지스트리(`vehicle.tables`) 동적 선택
- [x] **V9 종합 진단서** — 리콜+불만+매뉴얼 종합 (`/api/agent/report`, 집계=결정적 SQL)
- [x] **V10 이미지 진단→부품** — 사진(Vision+OCR)→매뉴얼 진단→부품 명세+샘플 견적
- [x] **V11 고도화** — text-to-SQL 자기수정 + 작업별 모델 라우팅(SQL=강한모델, 답변=FT)
- [ ] **V12 (옵션)** — vLLM provider · 임베딩 파인튜닝 · 온디바이스 음성(Whisper) · 라이브 배포 · GraphRAG 통합

> GraphRAG는 현재 **설계서**만 존재(미구현). 실제 RAG는 벡터+BM25 하이브리드. 고도화 방향은 [docs/GRAPHRAG_VEHICLE.md](docs/GRAPHRAG_VEHICLE.md).
...
- **데이터** — Apache Tika 멀티포맷 인제스트(+한국어 HWP/HWPX), 청킹(fixed/recursive/semantic), 티어드 저장(JSON→Parquet)
...
플랫폼 심화 문서(임베딩 비교·청킹·하이브리드·리랭킹·pgvector·보안·운영 등)와 엔지니어링 학습 노트는 **[docs/README.md](docs/README.md)** 색인 참고. (원본 플랫폼 README는 상위 레포 [dea980/miniwatson](https://github.com/dea980/miniwatson)에 보존)
