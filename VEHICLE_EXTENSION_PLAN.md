...
| RAG/Agent 핵심기술 연구와 고도화 | 기존 RAG 도메인화 + Agent 오케스트레이션 | P1, P4 |
| 자동차 밸류체인 NLP 서비스 (판매/제조/A·S) | 도메인 네임스페이스 + A/S 매뉴얼 검색 | P0, P1 |
| MLOps 연계, Inference API 설계와 구현 | vLLM provider + OpenAI 호환 API, CI | P3, P4 |
...
파인튜닝과 경량화는 **Python ML 사이드카**로 분리한 뒤,
...
- [ ] 청킹/리랭커 도메인 A/B (표와 수치가 많은 매뉴얼 → recursive vs semantic 비교)
...
- **Base 모델**: `Qwen2.5-1.5B-Instruct` (Apache-2.0, 한국어 양호, 로컬 학습과 추론이 현실적).
